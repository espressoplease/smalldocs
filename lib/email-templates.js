...
function billingState(input) {
  input = input || {};
  const type = cleanLine(input.type);
  const accountName = cleanLine(input.accountName, 'your Cloud account');
  const billingUrl = safeUrl(input.billingUrl);
  const billingOrigin = new URL(billingUrl).origin;
  const termsUrl = safeUrl(input.termsUrl || billingOrigin + '/legal');
  const cancellationUrl = safeUrl(input.cancellationUrl || billingOrigin + '/cancellation');
  const accessEndsAt = cleanLine(input.accessEndsAt);
  const deletionDate = cleanLine(input.deletionDate);
  const definitions = {
    subscription_started: {
      subject: 'SmallDocs Cloud is active for ' + accountName,
      heading: 'Cloud subscription active',
      lead: accountName + ' is ready to use.',
      details: [
        accessEndsAt ? 'The subscription renews monthly. The next billing date is ' + accessEndsAt + '.' :
          'The subscription renews monthly until cancelled.',
        'You can cancel future renewal from Cloud settings.',
        'A first Personal subscription bought as a consumer can be refunded when requested within 14 days.',
      ],
      action: 'Open Cloud settings',
    },
    payment_failed: {
      subject: 'Payment failed for ' + accountName,
      heading: 'Payment needs attention',
      lead: 'We could not collect payment for ' + accountName + '.',
      details: [
        accessEndsAt ? 'You can keep editing until ' + accessEndsAt + '.' : '',
        'After that, the account becomes read-only and you can still open and export documents.',
        deletionDate ? 'If payment remains unpaid, Cloud documents will be permanently deleted on ' +
          deletionDate + '.' : '',
      ],
      action: 'Update billing',
    },
    payment_recovered: {
      subject: 'Payment received for ' + accountName,
      heading: 'Payment received',
      lead: 'Payment for ' + accountName + ' has been received.',
      details: ['Editing is available again, and the scheduled deletion has been cancelled.'],
      action: 'View billing',
    },
    payment_read_only: {
      subject: accountName + ' is now read-only',
      heading: 'Cloud account is read-only',
      lead: 'The payment grace period for ' + accountName + ' has ended.',
      details: [
        'You can still open and export documents, but you cannot save changes to Cloud.',
        deletionDate ? 'If payment remains unpaid, Cloud documents will be permanently deleted on ' +
          deletionDate + '.' : '',
      ],
      action: 'Update billing',
    ...
    cancellation_scheduled: {
      subject: 'Cancellation scheduled for ' + accountName,
      heading: 'Cancellation scheduled',
      lead: accountName + ' will remain active through the paid period.',
      details: [
        accessEndsAt ? 'Editing remains available until ' + accessEndsAt + '.' : '',
        'After that, the account becomes read-only and you can still open and export documents.',
        deletionDate ? 'Cloud documents will be permanently deleted on ' + deletionDate +
          ' unless the subscription is restarted.' : '',
      ],
      action: 'View billing',
    },
    cancellation_reversed: {
      subject: 'Subscription will continue for ' + accountName,
      heading: 'Subscription will continue',
      lead: 'The scheduled cancellation for ' + accountName + ' has been cancelled.',
      details: ['Editing remains available and the scheduled deletion has been cancelled.'],
      action: 'View billing',
    },
    cancellation_effective: {
      subject: accountName + ' is now read-only',
      heading: 'Subscription has ended',
      lead: accountName + ' is now read-only.',
      details: [
        'You can still open and export documents, but you cannot save changes to Cloud.',
        deletionDate ? 'Cloud documents will be permanently deleted on ' + deletionDate +
          ' unless the subscription is restarted.' : '',
    ...
    deletion_warning: {
      subject: 'Cloud documents in ' + accountName + ' will be deleted' +
        (deletionDate ? ' on ' + deletionDate : ' soon'),
      heading: 'Cloud documents will be deleted soon',
      lead: 'Cloud documents in ' + accountName + ' are scheduled for permanent deletion.',
      details: [
        deletionDate ? 'Deletion is scheduled for ' + deletionDate + '.' : '',
        'Export any documents you need. To prevent deletion, update payment or restart the ' +
  ...
  if (!definition) throw new Error('unsupported billing email type');
  const details = definition.details.filter(Boolean);
  const text = definition.lead + '\n\n' + details.join('\n') + '\n\n' +
    definition.action + ':\n' + billingUrl +
    (type === 'subscription_started' ? '\n\nTerms:\n' + termsUrl +
      '\n\nCancellation and refunds:\n' + cancellationUrl : '');
  const detailHtml = details.map((detail) =>
    '<p style="border-top:1px solid #e7e2da;color:#56504a;font-size:13px;' +
      'margin:0;padding:9px 0">' + escapeHtml(detail) + '</p>'
  ).join('');
  const html = documentShell(definition.lead,
    heading(definition.heading) + paragraph(definition.lead, '0 0 14px') + detailHtml +
    button(definition.action, billingUrl) + (type === 'subscription_started'
      ? '<p style="color:#56504a;font-size:13px;margin:14px 0 0">' +
        '<a href="' + escapeHtml(termsUrl) + '">Terms</a> &nbsp; ' +
        '<a href="' + escapeHtml(cancellationUrl) + '">Cancellation and refunds</a></p>'
      : ''));
...
module.exports = { cleanLine, cleanMultiline, escapeHtml, safeUrl, signInCode, workspaceInvitation,
  documentNotification, billingState };
