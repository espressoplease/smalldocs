...
function safeReturnPath(value) {
  ...
  if (/[\u0000-\u001f\u007f]/.test(value)) return '/cloud/admin';
  try {
    const parsed = new URL(value, 'https://smalldocs.invalid');
    if (parsed.origin !== 'https://smalldocs.invalid') return '/cloud/admin';
...
async function defaultTransport(input) {
  ...
  const timeoutMs = input.timeoutMs == null ? DEFAULT_PROVIDER_TIMEOUT_MS : input.timeoutMs;
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1) {
    throw new OAuthError('invalid_provider_timeout', 'provider timeout must be a positive integer');
  }
  ...
  try {
    ...
    try { json = await response.json(); } catch (error) {
      if (controller.signal.aborted) {
        throw new OAuthError('provider_unavailable', 'provider request timed out');
      }
    }
...
function createGoogleOAuth(options) {
  ...
  async function providerRequest(input) {
    return transport(Object.assign({}, input, { timeoutMs: providerTimeoutMs }));
  }
  async function googleJwks(keyId) {
    ...
    cachedJwks = await resolveJwks(options.jwks, providerRequest, keyId);
  ...
  return {
    ...
    async callback(input) {
      ...
      const response = normalizeTransportResponse(await providerRequest({
...
function createGitHubOAuth(options) {
  ...
  return {
    ...
    async callback(input) {
      ...
      const exchanged = normalizeTransportResponse(await providerRequest({
      ...
      const userResponse = normalizeTransportResponse(await providerRequest({
      ...
      const emailResponse = normalizeTransportResponse(await providerRequest({
