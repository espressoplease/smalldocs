...
class CloudBillingStore {
  ...
  computeEntitlements(workspaceId, usageInput) {
    ...
    const memberLimit = subscription.plan === 'personal'
      ? 1
      : configured.maxMembers == null
        ? subscription.seatQuantity
        : Math.min(subscription.seatQuantity, configured.maxMembers);
    const limits = {
