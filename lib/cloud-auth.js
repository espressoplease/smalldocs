...
class CloudAuthStore {
  ...
  getUser(userId) {
    const user = this.db.prepare(`
      SELECT id, display_name, created_at_ms, disabled_at_ms FROM cloud_auth_users WHERE id = ?
    `).get(userId);
    if (!user) return null;
  ...
  updateUserProfile(input) {
    ...
    this.db.prepare('UPDATE cloud_auth_users SET display_name = ? WHERE id = ?')
      .run(displayName, input.userId);
