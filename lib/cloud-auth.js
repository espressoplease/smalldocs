...
function normalizeDisplayName(value) {
  if (typeof value !== 'string') throw new AuthError('invalid_request', 'display name must be a string');
  const name = value.trim().replace(/\s+/g, ' ');
  if (!name || name.length > 128) {
    throw new AuthError('invalid_request', 'display name must contain 1 to 128 characters');
  }
  return name;
}

function profileFromDisplayName(value) {
...
function normalizeProfileNames(firstValue, lastValue) {
  if (typeof firstValue !== 'string' || (lastValue != null && typeof lastValue !== 'string')) {
    throw new AuthError('invalid_request', 'first and last name must be strings');
  }
  const firstName = firstValue.trim().replace(/\s+/g, ' ');
  const lastName = String(lastValue || '').trim().replace(/\s+/g, ' ') || null;
  if (!firstName) throw new AuthError('invalid_request', 'first name is required');
  const displayName = [firstName, lastName].filter(Boolean).join(' ');
  if (firstName.length > 128 || (lastName && lastName.length > 128) || displayName.length > 128) {
    throw new AuthError('invalid_request', 'name must contain at most 128 characters');
  }
...
class CloudAuthStore {
  ...
  _createSchema() {
    ...
    if (!userColumns.some((column) => column.name === 'display_name')) {
      this.db.exec('ALTER TABLE cloud_auth_users ADD COLUMN display_name TEXT');
    }
  ...
  getUser(userId) {
    const user = this.db.prepare(`
      SELECT id, display_name, first_name, last_name, created_at_ms, disabled_at_ms
    ...
    `).all(userId);
    let profile = { displayName: user.display_name, firstName: user.first_name, lastName: user.last_name };
    if (!profile.firstName && profile.displayName) profile = profileFromDisplayName(profile.displayName);
    return {
      id: user.id,
  ...
  updateUserProfile(input) {
    ...
    const hasSeparateNames = Object.prototype.hasOwnProperty.call(input, 'firstName') ||
      Object.prototype.hasOwnProperty.call(input, 'lastName');
    const profile = hasSeparateNames
      ? normalizeProfileNames(input.firstName, input.lastName)
      : profileFromDisplayName(input.displayName);
    this.db.prepare(`
      UPDATE cloud_auth_users SET display_name = ?, first_name = ?, last_name = ? WHERE id = ?
    `).run(profile.displayName, profile.firstName, profile.lastName, input.userId);
