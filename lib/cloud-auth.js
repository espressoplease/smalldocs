...
const { CURRENT_TERMS_VERSION } = require('./cloud-terms');
...
function normalizeProfileNames(firstValue, lastValue) {
  if (typeof firstValue !== 'string' || typeof lastValue !== 'string') {
    throw new AuthError('invalid_request', 'first and last name must be strings');
  }
  const firstName = firstValue.trim().replace(/\s+/g, ' ');
  const lastName = lastValue.trim().replace(/\s+/g, ' ');
  if (!firstName || !lastName) {
    throw new AuthError('invalid_request', 'first and last name are required');
  }
  if (firstName.length > 128 || lastName.length > 128) {
    throw new AuthError('invalid_request', 'names must contain at most 128 characters');
  }
...
class CloudAuthStore {
  ...
  _createSchema() {
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS cloud_auth_terms_acceptances (
        user_id TEXT NOT NULL REFERENCES cloud_auth_users(id) ON DELETE CASCADE,
        terms_version TEXT NOT NULL,
        accepted_at_ms INTEGER NOT NULL,
        PRIMARY KEY(user_id, terms_version)
      );
      CREATE INDEX IF NOT EXISTS cloud_auth_terms_acceptances_time_idx
        ON cloud_auth_terms_acceptances(accepted_at_ms);
    ...
    `);
    const userColumns = this.db.pragma('table_info(cloud_auth_users)');
    if (!userColumns.some((column) => column.name === 'first_name')) {
      this.db.exec('ALTER TABLE cloud_auth_users ADD COLUMN first_name TEXT');
    }
    if (!userColumns.some((column) => column.name === 'last_name')) {
      this.db.exec('ALTER TABLE cloud_auth_users ADD COLUMN last_name TEXT');
    }
    if (userColumns.some((column) => column.name === 'display_name')) {
      this.db.exec('ALTER TABLE cloud_auth_users DROP COLUMN display_name');
    }
  }
  ...
  _signInWithVerifiedEmail(email, now) {
    const existing = this._identity('email', email);
    if (existing) {
      this._getActiveUser(existing.user_id);
      this.db.prepare('UPDATE cloud_auth_identities SET last_used_at_ms = ? WHERE id = ?')
        .run(now, existing.id);
      return { user: this.getUser(existing.user_id), created: false };
    }
    const userId = this._createUser(now);
    this._insertIdentity(userId, { provider: 'email', subject: email, verifiedEmail: email }, now);
    return { user: this.getUser(userId), created: true };
  }

  signInWithVerifiedEmail(value) {
    const email = normalizeEmail(value);
    const now = this.now();
    return this.db.transaction(() => this._signInWithVerifiedEmail(email, now)).immediate();
  }
  ...
  getUser(userId) {
    const user = this.db.prepare(`
      SELECT id, first_name, last_name, created_at_ms, disabled_at_ms
      FROM cloud_auth_users WHERE id = ?
    `).get(userId);
    if (!user) return null;
    ...
    `).all(userId);
    const termsAcceptance = this.db.prepare(`
      SELECT terms_version, accepted_at_ms FROM cloud_auth_terms_acceptances
      WHERE user_id = ? AND terms_version = ?
    `).get(userId, CURRENT_TERMS_VERSION) || null;
    return {
      id: user.id,
      ...
      termsVersion: termsAcceptance ? termsAcceptance.terms_version : null,
      termsAcceptedAtMs: termsAcceptance ? termsAcceptance.accepted_at_ms : null,
      identities,
    };
  }
  ...
  hasAcceptedCurrentTerms(userId) {
    ...
    return Boolean(this.db.prepare(`
      SELECT 1 FROM cloud_auth_terms_acceptances
      WHERE user_id = ? AND terms_version = ?
    `).get(userId, CURRENT_TERMS_VERSION));
  }

  acceptCurrentTerms(input) {
    input = input || {};
    this._getActiveUser(input.userId);
    if (input.termsVersion !== CURRENT_TERMS_VERSION) {
      throw new AuthError('invalid_terms_version', 'the current Terms version is required');
    }
    const acceptedAtMs = this.now();
    this.db.prepare(`
      INSERT OR IGNORE INTO cloud_auth_terms_acceptances
        (user_id, terms_version, accepted_at_ms)
      VALUES (?, ?, ?)
    `).run(input.userId, CURRENT_TERMS_VERSION, acceptedAtMs);
    const row = this.db.prepare(`
      SELECT terms_version, accepted_at_ms FROM cloud_auth_terms_acceptances
      WHERE user_id = ? AND terms_version = ?
    `).get(input.userId, CURRENT_TERMS_VERSION);
    return { termsVersion: row.terms_version, acceptedAtMs: row.accepted_at_ms };
  }

  _requireCurrentTerms(userId) {
    if (!this.hasAcceptedCurrentTerms(userId)) {
      throw new AuthError('terms_acceptance_required', 'the current Terms must be accepted');
    }
  }
  ...
  updateUserProfile(input) {
    input = input || {};
    this._getActiveUser(input.userId);
    const profile = normalizeProfileNames(input.firstName, input.lastName);
    this.db.prepare(`
      UPDATE cloud_auth_users SET first_name = ?, last_name = ? WHERE id = ?
    `).run(profile.firstName, profile.lastName, input.userId);
  ...
  approveDeviceAuthorization(input) {
    input = input || {};
    this._getActiveUser(input.userId);
    this._requireCurrentTerms(input.userId);
  ...
  pollDeviceAuthorization(input) {
    ...
    return this.db.transaction(() => {
      ...
      if (!this.hasAcceptedCurrentTerms(row.approved_by_user_id)) {
  ...
  refreshCliCredential(input) {
    ...
    const result = this.db.transaction(() => {
      ...
      this._requireCurrentTerms(credential.user_id);
  ...
  countRateLimit(input) {
    input = input || {};
    const action = typeof input.action === 'string' ? input.action.trim() : '';
    const key = typeof input.key === 'string' ? input.key.trim() : '';
    const windowMs = Number(input.windowMs);
    if (!action || !key || !Number.isFinite(windowMs) || windowMs < 1) {
      throw new Error('action, key, and positive windowMs are required');
    }
    const subjectHash = this._hash('rate:' + action, key);
    return this.db.prepare(`
      SELECT COUNT(*) AS count FROM cloud_auth_rate_events
      WHERE action = ? AND subject_hash = ? AND created_at_ms > ?
    `).get(action, subjectHash, this.now() - windowMs).count;
  }
  ...
  consumeEmailCode(input) {
    ...
    return this.db.transaction(() => {
      ...
      if (row.purpose === 'link') {
        ...
        const signedIn = this._signInWithVerifiedEmail(row.email, now);
  ...
  authenticateSession(token) {
    ...
    const now = this.now();
    const row = this.db.prepare(`
      SELECT id, user_id, created_at_ms, expires_at_ms, revoked_at_ms
    ...
    return { ok: true, session: { id: row.id, createdAtMs: row.created_at_ms,
      expiresAtMs: row.expires_at_ms }, user };
  }
