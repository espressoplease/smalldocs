...
function validateCloudDeploymentConfig(env) {
  ...
  const publicMode = String(env.CLOUD_PUBLIC_MODE || 'hidden').trim().toLowerCase();
  if (!['off', 'staging', 'production'].includes(mode)) {
    throw new CloudDeploymentConfigError(['CLOUD_MODE must be off, staging, or production']);
  }
  if (!['hidden', 'enabled'].includes(publicMode)) {
    throw new CloudDeploymentConfigError(['CLOUD_PUBLIC_MODE must be hidden or enabled']);
  }
  if (!DEPLOYED_MODES.has(mode)) {
    return { mode, enabled: false, publicMode, publicEnabled: publicMode === 'enabled' };
  }
  ...
  const testLoginSettings = ['CLOUD_TEST_LOGIN_ENABLED', 'CLOUD_TEST_LOGIN_EMAILS',
    'CLOUD_TEST_LOGIN_SECRET_FILE'].filter((name) => present(env, name));
  if (mode === 'production' && testLoginSettings.length) {
    problems.push('staging test login settings must be unset in production');
  }
  if (mode === 'staging' && testLoginSettings.length) {
    if (env.CLOUD_TEST_LOGIN_ENABLED !== '1') {
      problems.push('CLOUD_TEST_LOGIN_ENABLED must be 1 when staging test login is configured');
    }
    if (!present(env, 'CLOUD_TEST_LOGIN_EMAILS')) {
      problems.push('CLOUD_TEST_LOGIN_EMAILS is required for staging test login');
    }
    if (!present(env, 'CLOUD_TEST_LOGIN_SECRET_FILE')) {
      problems.push('CLOUD_TEST_LOGIN_SECRET_FILE is required for staging test login');
    } else if (!path.isAbsolute(env.CLOUD_TEST_LOGIN_SECRET_FILE)) {
      problems.push('CLOUD_TEST_LOGIN_SECRET_FILE must be an absolute path');
    }
  }
  ...
  return { mode, enabled: true, publicMode, publicEnabled: publicMode === 'enabled', origin: origin.origin,
