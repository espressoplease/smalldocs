...
class CloudStore {
  ...
  async createTeamWorkspace(input) {
    ...
    return this.db.transaction(() => this._insertWorkspaceAndProject(prepared)).immediate();
  }
  ...
  async listWorkspaces(userId) {
    ...
    for (const row of rows) {
      workspaces.push({ id: row.id, kind: row.kind,
        name: await this._workspaceName(row), role: row.role });
    }
    return workspaces;
  }
  ...
  async listProjects(userId, workspaceId) {
    ...
    const projects = [];
    for (const row of rows) {
      projects.push({ id: row.id, workspaceId: row.workspace_id,
        name: await this._decryptProjectName(row),
  ...
  async createInvitation(input) {
    ...
    this.db.transaction(() => {
      const currentRole = this._workspaceRole(input.userId, input.workspaceId);
      if (currentRole !== 'owner' && currentRole !== 'admin') throw new CloudError('permission_denied');
      if (role === 'admin' && currentRole !== 'owner') throw new CloudError('permission_denied');
      grants.forEach((grant) => {
  ...
  async listWorkspaceInvitations(input) {
    ...
    for (const row of rows) {
      invitations.push({
        id: row.id,
        email: await this._keyOperation(() => this.keyProvider.decryptWorkspaceValue(
  ...
  async getDocument(input) {
    ...
    return {
      ...this._documentResult(document.id, content.metadata, row.revision_number, row.id,
        document.updated_at_ms, { id: document.project_id, name: await this._decryptProjectName(project) }),
