...
class LocalKeyProvider {
  ...
  encryptWorkspaceName(workspaceId, name) {
    return encryptAead(this.masterKey, Buffer.from(name), this._rootAad('workspace-name', workspaceId, 1), this.randomBytes);
  }

  decryptWorkspaceName(workspaceId, encrypted) {
    return decryptAead(this.masterKey, encrypted.nonce, encrypted.ciphertext,
      this._rootAad('workspace-name', workspaceId, 1)).toString('utf8');
  }
}
