...
class CloudStore {
  ...
  async listDocuments(input) {
    ...
    for (const project of projects) {
      const projectName = await this._decryptProjectName(project);
      this._requireProject(input.userId, project.id, 'viewer');
      const rows = this.db.prepare(`
      ...
      for (const row of rows) {
        const content = await this._decryptRevision(row);
        this._requireDocument(input.userId, row.document_id, 'viewer');
  ...
  async listDeletedDocuments(input) {
    ...
    for (const row of rows) {
      ...
      this._requireDocument(input.userId, row.document_id, 'editor', true);
      results.push({
