...
class CloudStore {
  ...
  listWorkspaceInvitations(input) {
    ...
    `).all(input.workspaceId, now).map((row) => ({
      id: row.id,
      email: this.keyProvider.decryptWorkspaceValue(row.workspace_id, 'invitation-email', {
  ...
  listDeletedDocuments(input) {
    ...
    const rows = this.db.prepare(`
      ...
      LEFT JOIN cloud_project_grants g
        ON g.project_id = d.project_id AND g.user_id = m.user_id AND g.revoked_at_ms IS NULL
      WHERE m.user_id = ? AND m.status = 'active' AND p.deleted_at_ms IS NULL
        AND d.deleted_at_ms IS NOT NULL AND d.purge_after_ms > ?
        AND (? IS NULL OR d.workspace_id = ?)
        AND (m.role IN ('owner', 'admin') OR g.role = 'editor')
    ...
    return rows.map((row) => {
      ...
      return {
        ...this._documentResult(row.document_id, content.metadata, row.revision_number, row.id,
          row.updated_at_ms, { id: row.project_id, name: this._decryptProjectName({
  ...
  listDocumentsPage(input) {
    input = input || {};
    return this.pageDocuments(this.listDocuments(input), input);
  }
  ...
  search(input) {
    ...
    results.slice(0, limit).forEach((result) => {
      if (!authorizedProjects.has(result.project.id)) {
        this._requireProject(input.userId, result.project.id, 'viewer');
        authorizedProjects.add(result.project.id);
      }
    });
  ...
  restoreDeletedDocument(input) {
    ...
    `).run(now, document.id, input.expectedHeadRevisionId, now);
    if (result.changes !== 1) throw new CloudError('resource_unavailable');
    this._audit({ workspaceId: document.workspace_id, projectId: document.project_id,
      userId: input.userId, action: 'document.restore', resourceId: document.id });
    return this.getDocument({ userId: input.userId, documentId: document.id });
  }
  ...
  exportWorkspace(input) {
    ...
    const workspace = this.listWorkspaces(input.userId).find((item) => item.id === input.workspaceId);
    if (!workspace) throw new CloudError('resource_unavailable');
    const projects = this.listProjects(input.userId, input.workspaceId);
    const documents = [];
    projects.forEach((project) => {
      this.listDocuments({ userId: input.userId, projectId: project.id }).forEach((summary) => {
        const current = this.getDocument({ userId: input.userId, documentId: summary.id });
        const revisions = input.includeRevisions === false ? [] : this.listRevisions({
          userId: input.userId, documentId: summary.id,
        }).map((revision) => this.getDocument({ userId: input.userId,
