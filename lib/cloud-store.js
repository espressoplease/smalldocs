...
class CloudStore {
  ...
  createDocumentNotification(input) {
    input = input || {};
    ...
    const documents = documentIds.map((documentId) => {
      const document = this._requireDocument(input.userId, documentId, 'editor').row;
      if (document.created_by_user_id !== input.userId) throw new CloudError('permission_denied');
      return document;
    });
    ...
    const requestDigest = this._digest({ documentIds, recipientUserIds });
    ...
    return this.db.transaction(() => {
      ...
      documents.forEach((document) => {
        const current = this._requireDocument(input.userId, document.id, 'editor').row;
        if (current.created_by_user_id !== input.userId || current.workspace_id !== workspaceId) {
          throw new CloudError('permission_denied');
        }
        recipientUserIds.forEach((recipientUserId) => {
      ...
      this.db.prepare(`
        INSERT INTO cloud_notification_batches
          (id, workspace_id, actor_user_id, actor_credential_id, idempotency_key,
