...
class CloudStore {
  ...
  _requireWorkspaceRole(userId, workspaceId, allowedRoles) {
    const role = this._workspaceRole(userId, workspaceId);
    if (!role) throw new CloudError('resource_unavailable');
    if (!allowedRoles.includes(role)) throw new CloudError('permission_denied');
    return role;
  }
  ...
  async createProject(input) {
    input = input || {};
    this._requireWorkspaceRole(input.userId, input.workspaceId, ['owner', 'admin']);
    const name = requireText(input.name, 'name', 512);
    const now = this.now();
    const projectId = this._uuid();
    const projectKey = this.randomBytes(32);
  ...
  addWorkspaceMember(input) {
    input = input || {};
    const actorRole = this._requireWorkspaceRole(
      input.actorUserId, input.workspaceId, ['owner', 'admin']);
  ...
  listWorkspaceOwnerUserIds(workspaceId) {
    return this.db.prepare(`
      SELECT user_id FROM cloud_workspace_memberships
      WHERE workspace_id = ? AND role = 'owner' AND status = 'active'
      ORDER BY created_at_ms, user_id
    `).all(workspaceId).map((owner) => owner.user_id);
  }

  async getWorkspaceBillingContext(workspaceId) {
    const row = this.db.prepare(`
      SELECT * FROM cloud_workspaces WHERE id = ?
    `).get(workspaceId);
    if (!row) return null;
    return {
      id: row.id,
      kind: row.kind,
      name: await this._workspaceName(row),
      ownerUserIds: this.listWorkspaceOwnerUserIds(workspaceId),
    };
  }
  ...
  transferWorkspaceOwnership(input) {
    ...
    return this.db.transaction(() => {
      this._requireWorkspaceRole(input.actorUserId, input.workspaceId, ['owner']);
  ...
  deleteWorkspace(input) {
    ...
    return this.db.transaction(() => {
      this._requireWorkspaceRole(input.userId, input.workspaceId, ['owner']);
  ...
  purgeWorkspaceForBilling(input) {
    input = input || {};
    const workspaceId = requireText(input.workspaceId, 'workspace_id', 200);
    return this.db.transaction(() => {
      const workspace = this.db.prepare('SELECT id FROM cloud_workspaces WHERE id = ?')
        .get(workspaceId);
      if (!workspace) return { purged_count: 0 };
      this.db.prepare(`
        DELETE FROM cloud_idempotency_records
        WHERE resource_id = ?
          OR resource_id IN (SELECT id FROM cloud_projects WHERE workspace_id = ?)
      ...
      this.db.prepare('DELETE FROM cloud_documents WHERE workspace_id = ?').run(workspaceId);
      this.db.prepare('DELETE FROM cloud_audit_events WHERE workspace_id = ?').run(workspaceId);
      const removed = this.db.prepare('DELETE FROM cloud_workspaces WHERE id = ?')
        .run(workspaceId);
      return { purged_count: removed.changes };
    }).immediate();
  }
  ...
  async createInvitation(input) {
    ...
    if (!actorRole) throw new CloudError('resource_unavailable');
    ...
    this.db.transaction(() => {
      ...
      if (!currentRole) throw new CloudError('resource_unavailable');
  ...
  grantProject(input) {
    input = input || {};
    this._requireWorkspaceRole(input.actorUserId, input.workspaceId, ['owner', 'admin']);
  ...
  async saveTargetRevision(input) {
    ...
    const idempotency = {
      ...
      idempotencyKey: input.idempotencyKey,
      request: { targetRevisionId, targetMarkdown: input.targetMarkdown,
        markdown: proposedMarkdown, filename: input.filename },
    };
    const replay = this._idempotentReplay(idempotency);
    if (replay.found) return replay.response;
    const target = this.db.prepare(`
    ...
    if (!target && typeof input.targetMarkdown !== 'string') {
    ...
    const targetRecovered = !target;
    const targetContent = target
      ? await this._decryptRevision(target)
      : { markdown: requireMarkdown(input.targetMarkdown),
        metadata: deriveMetadata(input.targetMarkdown, input.filename) };
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      const currentContent = target && current.id === target.id
  ...
  async updateDocumentTags(input) {
    ...
    if (input.targetRevisionId) {
      try {
        opened = await this.getDocument({ userId: input.userId, documentId: input.documentId,
          revisionId: input.targetRevisionId });
      } catch (error) {
        if (!error || error.code !== 'resource_unavailable') throw error;
        if (typeof input.targetMarkdown !== 'string') {
          const head = this.getDocumentHead({ userId: input.userId, documentId: input.documentId });
          throw new CloudError('target_too_old', null, {
        ...
        opened = { id: input.documentId, markdown: requireMarkdown(input.targetMarkdown),
          filename: input.filename || deriveMetadata(input.targetMarkdown).filename };
      }
    } else {
      opened = await this.getDocument({ userId: input.userId, documentId: input.documentId });
    }
    ...
    if (input.targetRevisionId) {
      return this.saveTargetRevision({
        userId: input.userId,
        ...
        idempotencyKey: input.idempotencyKey,
        maxMarkdownBytes: input.maxMarkdownBytes,
