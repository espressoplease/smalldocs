...
function detectExportFontChains(root) {
  var host = root || document.body;
  var cs;
...
async function drawShapeViaRaster(slide, s, idx, grid, slideW, slideH, stage) {
  ...
  if (!bb) {
    var declared = ACTIVE.shapes.bboxOf(s);
...
async function rasterizeRichBlock(wrapper) {
  ...
  if (wrapper.classList.contains('sdoc-mermaid')) {
    if (ACTIVE.rasterizeMermaidWrapper) {
      var d = await ACTIVE.rasterizeMermaidWrapper(wrapper);
  ...
  if (wrapper.classList.contains('sdocs-math-display') ||
    ...
    if (ACTIVE.rasterizeMathElement) {
      var res = await ACTIVE.rasterizeMathElement(wrapper);
      if (!res) return null;
...
async function drawSlide(ctx) {
  ...
  if (!ctx.pptxGenJS) throw new Error('PptxGenJS not loaded');
  ...
  try {
    ...
    var fontChains = detectExportFontChains(ctx.root);
