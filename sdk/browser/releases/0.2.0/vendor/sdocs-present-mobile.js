...
(function () {
  ...
  function createMobilePresenter(options) {
  options = options || {};
  var ZM = options.zoomMath || window.SDocZoomMath;
  ...
  function applyFormFactor() {
    ...
    if (typeof options.refit === 'function') options.refit();
    cacheRects();
    clampPan();
  ...
  function onOpen(refs) {
    ...
    if (touchDevice()) {
      ...
      setHTML(ghostClose, '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>');
      ghostClose.addEventListener('click', function () { if (typeof options.close === 'function') options.close(); });
  ...
  function onRender(index, slideCount) {
    if (!active) return;
    curIndex = typeof index === 'number' ? index : curIndex;
    total = typeof slideCount === 'number' ? slideCount : total;
  ...
  return { onOpen: onOpen, onRender: onRender, onClose: onClose };
  }

  var defaultMobile = createMobilePresenter({
    zoomMath: window.SDocZoomMath,
    go: function (index) { if (window.SDocPresent) window.SDocPresent.go(index); },
    close: function () { if (window.SDocPresent) window.SDocPresent.close(); },
    refit: function () { if (window.SDocPresent) window.SDocPresent.refit(); },
  });
  window.SDocPresentMobile = Object.assign({ create: createMobilePresenter }, defaultMobile);
})();
