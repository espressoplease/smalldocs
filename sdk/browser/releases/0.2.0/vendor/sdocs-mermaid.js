...
(function (exports) {
  'use strict';
  function createMermaid(environment) {
  var env = environment || {};
  var window = env.window || globalThis.window;
  var document = env.document || window.document;
  var S = env.sdocs || window.SDocs || {};
  var rootSource = env.root;
  var scopeSource = env.scope;
  var setSvgHTML = typeof env.setSvgHTML === 'function'
    ? env.setSvgHTML
    : function (element, source) { element.innerHTML = source; };
  var parseSvg = typeof env.parseSvg === 'function'
    ? env.parseSvg
    : function (source) { return new DOMParser().parseFromString(source, 'image/svg+xml'); };
  var externallyLoadMermaid = typeof env.loadMermaid === 'function' ? env.loadMermaid : null;
  ...
  function readerRoot() {
    if (typeof rootSource === 'function') return rootSource();
    return rootSource || document.getElementById('_sd_rendered');
  }

  function queryScope() {
    if (typeof scopeSource === 'function') return scopeSource();
    return scopeSource || document;
  }
  ...
  function isDark() {
    if (typeof env.isDark === 'function') return Boolean(env.isDark());
    var root = readerRoot();
    return document.documentElement.dataset.theme === 'dark'
      || Boolean(root && root.closest && root.closest('[data-theme="dark"]'));
  }

  function normalizeColor(value) {
    var text = String(value || '');
    return /^#[0-9a-f]{3,8}$/i.test(text) ? text.toLowerCase() : text;
  }
  ...
  function readThemeVars() {
    ...
    var dark = isDark();
    var bg      = normalizeColor(cssVar('--md-bg')         || (dark ? '#2c2a26' : '#ffffff'));
    var color   = normalizeColor(cssVar('--md-color')      || (dark ? '#e7e5e2' : '#1c1917'));
    var blockBg = normalizeColor(cssVar('--md-block-bg')   || (dark ? '#1f1d1a' : '#f4f1ed'));
    var blockTx = normalizeColor(cssVar('--md-block-text') || (dark ? '#a8a29e' : '#6b6560'));
    ...
    var accent    = normalizeColor(cssVar('--md-link-color') || (dark ? '#60a5fa' : '#2563eb'));
  ...
  function initMermaid(mermaid) {
    mermaid.initialize({
      ...
      themeCSS: [
        ...
        'min-width:0;width:max-content!important;max-width:240px!important;',
        'white-space:normal!important;padding:2px 8px;border-radius:8px;',
        'background-color:var(--md-block-bg,#f4f1ed);',
        'border:1px solid color-mix(in srgb,var(--md-block-text,#6b6560) 28%,transparent);',
        'box-sizing:border-box;',
        '}',
        'foreignObject > div { min-width:120px;text-align:center;box-sizing:border-box; }',
  ...
  function loadMermaid() {
    if (externallyLoadMermaid) return externallyLoadMermaid();
  ...
  function renderSourceIntoWrapper(mermaid, wrapper, stage, rawSrc, themeVars) {
    ...
    return withTimeout(p, RENDER_TIMEOUT_MS)
      .then(function (out) {
        if (!isActive() || destroyed || !isNodeActive(wrapper)) return;
        var svg = (out && out.svg) || '';
        setSvgHTML(stage, sanitizeSvg(svg));
  ...
  function processMermaid(container) {
    if (!container || destroyed || !isActive()) return Promise.resolve();
    ...
    return loadMermaid().then(function (mermaid) {
      ...
      var jobs = capped.map(function (codeEl) {
        if (!isActive() || destroyed || !isNodeActive(codeEl)) return Promise.resolve();
        ...
        if (!target.parentNode) return Promise.resolve();
  ...
  function rethemeMermaid() {
    ...
    var wrappers = Array.prototype.slice.call(scope.querySelectorAll('.sdoc-mermaid'));

    // Slide-embedded diagrams sit in the shadow root of each slide shape;
    // querySelectorAll above doesn't cross shadow boundaries.
    var shadowRoots = [];
    Array.prototype.forEach.call(scope.querySelectorAll('.shape-md'), function (host) {
  ...
  var api = {
    processMermaid: processMermaid,
    rethemeMermaid: rethemeMermaid,
    sanitizeSvg: sanitizeSvg,
    stripDirectives: stripDirectives,
    readThemeVars: readThemeVars,
    destroy: function () {
      destroyed = true;
      if (S.SDocMermaidFocus && typeof S.SDocMermaidFocus.destroy === 'function') S.SDocMermaidFocus.destroy();
    },
  };
  return api;
  }

  exports.create = createMermaid;
  if (typeof window !== 'undefined' && window.SDocs && window.SDocs.__smallDocsApp) {
    var production = createMermaid({
      window: window,
      document: window.document,
      sdocs: window.SDocs,
      root: function () { return window.document.getElementById('_sd_rendered'); },
      scope: window.document,
    });
    window.SDocs.processMermaid = production.processMermaid;
    window.SDocs.rethemeMermaid = production.rethemeMermaid;
  }
})(typeof module !== 'undefined' && module.exports ? module.exports : (window.SDocMermaidCore = {}));
