...
function createStage(gridW, gridH, root) {
  var stageW = STAGE_W;
...
function drawShapePolygon(page, s, grid, bounds) {
  ...
  var polyPath = ACTIVE.renderer && ACTIVE.renderer.polyPath;
  var d;
  if (polyPath) {
    d = polyPath(s.points);
  } else {
...
function drawShapeChev(page, s, grid, bounds) {
  var path = ACTIVE.renderer && ACTIVE.renderer.chevPath;
  if (!path) return;
  drawGridSvgPath(page, path(s), s, grid, bounds);
}

function drawShapeCyl(page, s, grid, bounds) {
  var make = ACTIVE.renderer && ACTIVE.renderer.cylPaths;
  if (!make) return;
  var p = make(s);
  ...
  var lip = ACTIVE.shapes.cylLip(s);
  ...
  if (s.attrs && s.attrs.fill) {
    var lc = ACTIVE.shapes.cylLidColors(s.attrs.fill);
...
function drawShapeBub(page, s, grid, bounds) {
  var path = ACTIVE.renderer && ACTIVE.renderer.bubPath;
  if (!path) return;
  drawGridSvgPath(page, path(s), s, grid, bounds);
}

function drawShapeTab(page, s, grid, bounds) {
  var path = ACTIVE.renderer && ACTIVE.renderer.tabPath;
  if (!path) return;
  drawGridSvgPath(page, path(s), s, grid, bounds);
}

function drawShapeDoc(page, s, grid, bounds) {
  var make = ACTIVE.renderer && ACTIVE.renderer.docPaths;
  if (!make) return;
  var p = make(s);
  drawGridSvgPath(page, p.body, s, grid, bounds);
...
async function drawRichBlock(el, kind, stageRect, page, bounds, pdfDoc, op) {
  ...
  if (kind === 'chart') {
    ...
    if (!svg || !ACTIVE.rasterizeMermaidWrapper) return;
    target = svg;
    dataUrl = await ACTIVE.rasterizeMermaidWrapper(el);
  } else if (kind === 'math') {
    if (!ACTIVE.rasterizeMathElement) return;
    var res = await ACTIVE.rasterizeMathElement(el);
    if (res) {
      dataUrl = res.dataUrl;
...
function drawRotatedShapeText(container, shape, angle, grid, bounds, page, fonts) {
  ...
  var opts = {
    ...
    font: font,
    color: color,
    rotate: ACTIVE.pdfLib.degrees(pdfAngle),
  };
  try {
...
async function drawSlide(ctx) {
  var SDocShapes = ctx.shapes;
  var SDocShapeRender = ctx.renderer;
  if (!SDocShapes || !SDocShapeRender) throw new Error('Shape renderer not loaded');
  if (!ctx.pdfLib) throw new Error('pdf-lib not loaded');
  ...
  var st = createStage(grid.w, grid.h, ctx.root);
  try {
