...
(function (exports) {
  'use strict';
  var ACTIVE_FOCUS = typeof Symbol === 'function' ? Symbol.for('smalldocs.mermaid.focus') : '__sdocsMermaidFocus';

  function createFocus(environment) {
  var env = environment || {};
  var window = env.window || globalThis.window;
  var document = env.document || window.document;
  var navigator = window.navigator;
  var S = env.sdocs || window.SDocs || {};
  var ZM = env.zoomMath || window.SDocZoomMath;
  var setHTML = typeof env.setHTML === 'function'
    ? env.setHTML
  ...
  var CSS = [
    ...
    '}',
    '.sdoc-mermaid-tool-action {',
    '  width: auto; padding: 0 7px; gap: 4px;',
    '  font: 500 11px/1 ui-sans-serif, system-ui, sans-serif;',
    '}',
    '.sdoc-mermaid-tool-action-label { white-space: nowrap; }',
    ...
    '}',
    '.sdoc-mermaid-focus, .sdoc-mermaid-focus * { box-sizing: border-box; }',
  ...
  function flashCopyTick(btn) {
    ...
    replaceElementHTML(svg, CHECK_ICON_SVG);
    setTimeout(function () {
      var current = btn.querySelector('svg');
      if (current) replaceElementHTML(current, previous);
    }, 1500);
  }

  function replaceElementHTML(element, source) {
    if (!element || !element.parentNode) return;
    var holder = document.createElement('span');
    setHTML(holder, source);
    if (holder.firstElementChild) element.parentNode.replaceChild(holder.firstElementChild, element);
  }
  ...
  function buildCopyButton(wrapper) {
    ...
    setHTML(btn, COPY_ICON_SVG + '<span class="sdoc-mermaid-tool-action-label">Source</span>');
  ...
  function buildPngButton(wrapper) {
    var btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'sdoc-mermaid-tool-btn sdoc-mermaid-tool-action sdoc-mermaid-png-btn';
    btn.setAttribute('aria-label', 'Copy diagram as PNG');
    btn.title = 'Copy diagram as PNG';
    setHTML(btn, COPY_ICON_SVG + '<span class="sdoc-mermaid-tool-action-label">PNG</span>');
    btn.addEventListener('click', function (e) {
      e.stopPropagation();
      copyWrapperPng(wrapper, btn);
    });
    return btn;
  }
  ...
  function buildTools(wrapper) {
    ...
    if (controls.copy !== false) {
      ...
      tools.appendChild(buildPngButton(wrapper));
    }
    if (controls.fullscreen !== false) tools.appendChild(buildZoomButton(wrapper));
    return tools;
  }
  ...
  function open(sourceWrapper) {
    if (modal) close();
    var otherFocus = document[ACTIVE_FOCUS];
    if (otherFocus && otherFocus !== api && typeof otherFocus.close === 'function') otherFocus.close('superseded');
    ...
    if (controls.copy === false) {
      Array.prototype.forEach.call(topbar.querySelectorAll('[data-act="copy-text"], [data-act="copy-png"]'), function (button) { button.remove(); });
    }
    if (controls.download === false) {
      var saveButton = topbar.querySelector('[data-act="save-png"]');
    ...
    if (mountSurface) {
      managedLease = mountSurface(modal, {
        initialFocus: firstBtn,
        beforeClose: function (reason) { close(reason, true); },
      });
    } else {
      document.body.appendChild(modal);
      document.body.classList.add('sdoc-mermaid-focus-open');
    }
    document[ACTIVE_FOCUS] = api;
    ...
    if (!mountSurface && firstBtn && !S.embedMode) firstBtn.focus();
  }

  function close(reason, fromLease) {
    if (!modal) return;
    if (managedLease && !fromLease) {
      var lease = managedLease;
      managedLease = null;
      lease.close(reason || 'user');
      return;
    }
    ...
    if (document[ACTIVE_FOCUS] === api) delete document[ACTIVE_FOCUS];
    if (S.syncEmbedFocus) S.syncEmbedFocus();
  ...
  // Render one SVG to a PNG blob at 2x device scale, painting the surface
  // behind it so an image pasted into another app keeps the theme it had in
  // SmallDocs. Used by both the inline and fullscreen copy controls.
  function svgElementToPngBlob(svg, background, scale) {
  ...
  function svgToPngBlob(scale) {
    if (!svgWrap) return Promise.reject(new Error('no svg'));
    var svg = svgWrap.querySelector('svg');
    var background = modal ? getComputedStyle(modal).backgroundColor : '#ffffff';
    return svgElementToPngBlob(svg, background, scale);
  }

  function wrapperBackground(wrapper) {
    var background = wrapper ? getComputedStyle(wrapper).backgroundColor : '';
    if (!isTransparentColor(background)) return background;
    var rendered = document.getElementById('_sd_rendered');
    background = rendered ? getComputedStyle(rendered).backgroundColor : '';
    return isTransparentColor(background) ? '#ffffff' : background;
  }
  ...
  function flashLabel(btn, text) {
    var labelEl = btn.querySelector('.sdoc-mermaid-focus-action-label, .sdoc-mermaid-tool-action-label');
  ...
  function copyWrapperPng(wrapper, btn) {
    if (!window.ClipboardItem || !navigator.clipboard || !navigator.clipboard.write) {
      flashLabel(btn, 'Not supported');
      return;
    }
    var svg = wrapper && wrapper.querySelector('svg');
    svgElementToPngBlob(svg, wrapperBackground(wrapper), 2).then(function (blob) {
      return navigator.clipboard.write([new ClipboardItem({ 'image/png': blob })]);
    }).then(function () {
      flashCopyTick(btn);
    }).catch(function () {
  ...
  var api = {
    ...
    destroy: function () { close('destroy'); }
  };
  return api;
  }

  exports.create = createFocus;
  if (typeof window !== 'undefined' && window.SDocs && window.SDocs.__smallDocsApp && window.SDocZoomMath) {
    window.SDocs.SDocMermaidFocus = createFocus({
      window: window,
      document: window.document,
      sdocs: window.SDocs,
      zoomMath: window.SDocZoomMath,
    });
  }
})(typeof module !== 'undefined' && module.exports ? module.exports : (window.SDocMermaidFocusCore = {}));
