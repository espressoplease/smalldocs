...
//
// Dependencies are passed through setRuntime before rendering.
//
...
function injectCSS() {
  ...
  if (existing && existing.tagName === 'STYLE' && existing.dataset.smalldocsSdkVersion === '0.3.1') return;
  var style = document.createElement('style');
  style.id = CSS_ID;
  style.dataset.smalldocsSdkVersion = '0.3.1';
...
function contentToMarkdownNode(content, attrs) {
  ...
  var style = document.createElement('style');
  style.textContent = SHAPE_MD_SHADOW_CSS + (overrides ? '\n' + overrides : '');
...
function attachCodeCopyButtons(shadow) {
  ...
  for (var i = 0; i < pres.length; i++) {
    (function(pre) {
      ...
      setKnownHTML(btn, SHAPE_COPY_SVG);
      btn.title = 'Copy code';
      ...
      btn.addEventListener('click', function(e) {
        ...
        navigator.clipboard.writeText(text).then(function() {
          setKnownHTML(btn, SHAPE_CHECK_SVG);
          setTimeout(function() { setKnownHTML(btn, SHAPE_COPY_SVG); }, 1500);
        });
      });
      pre.appendChild(btn);
    })(pres[i]);
...
function attachShapeCopyButton(el, s, grid) {
  ...
  btn.addEventListener('click', function (e) {
    ...
    copyTextToClipboard(readShapeText(el), function () {
      ...
      setKnownHTML(btn, SHAPE_CHECK_SVG);
      setTimeout(function () { btn.classList.remove('copied'); setKnownHTML(btn, SHAPE_COPY_SVG); }, 1500);
    });
  });
  el.appendChild(btn);
  return btn;
...
// Resolve contrast after CSS-backed document colors reach the live stage.
...
function shapePaddingGridUnits(s) {
  ...
  var box = runtimeShapes().contentBox(s);
  if (!box) return 0;
...
function renderRect(s, grid) {
  ...
  if (s.content != null && s.content !== '') {
    ...
    var tb = runtimeShapes() && runtimeShapes().parseTextBox
      ? runtimeShapes().parseTextBox(s.attrs && s.attrs.textBox) : null;
    if (tb) {
      var inner = document.createElement('div');
...
function chevPath(s) {
  var tip = runtimeShapes().chevTip(s);
  var notch = runtimeShapes().chevNotch(s);
...
function cylPaths(s) {
  var lip = runtimeShapes().cylLip(s);
  var x = s.x, y = s.y, w = s.w, h = s.h;
  var rx = w / 2;
  var ry = lip / 2;
  var body = 'M ' + x + ' ' + (y + ry) +
...
function renderCyl(s) {
  ...
  if (s.attrs.fill) {
    var lc = runtimeShapes().cylLidColors(s.attrs.fill);
...
function bubPath(s) {
  var tail = runtimeShapes().bubTail(s);
  var x = s.x, y = s.y, w = s.w, h = s.h;
...
function tabPath(s) {
  var th = runtimeShapes().tabHeight(s);
  var x = s.x, y = s.y, w = s.w, h = s.h;
...
function docPaths(s) {
  var fold = runtimeShapes().docFold(s);
  var x = s.x, y = s.y, w = s.w, h = s.h;
  var body = 'M ' + x + ' ' + y +
             ' L ' + (x + w - fold) + ' ' + y +
             ' L ' + (x + w) + ' ' + (y + fold) +
...
function renderIcon(s) {
  ...
  var icons = runtimeIcons();
  var iconXml = (name && icons) ? icons[name] : null;
  if (!iconXml) {
