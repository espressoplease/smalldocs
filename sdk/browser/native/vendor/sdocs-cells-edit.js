...
(function (root, factory) {
  ...
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
    ...
    root.SDocCellsEdit = api;
    if (root.SDocs && root.SDocCells) api.installProduction(root);
  }
  ...
  if (!window || !document) throw new Error('Cells editor requires window and document');
  if (!CELLS) throw new Error('Cells editor requires the cells model');
  var portal = options.portal || document.body;
  var getStyle = options.getComputedStyle || window.getComputedStyle.bind(window);
  var matchMedia = options.matchMedia || (window.matchMedia && window.matchMedia.bind(window));
  var elementFromPoint = options.elementFromPoint || document.elementFromPoint.bind(document);
  var EventCtor = options.Event || window.Event;
  var attachments = [];
  var destroyed = false;
  ...
  function attach(wrapper, opts) {
    ...
    function makePointer(input, originCell) {
      ...
      function writeRef() {
        ...
        try { input.dispatchEvent(new EventCtor('input', { bubbles: true })); } catch (_) {}
        paintHighlight();
      }
    ...
    function begin(r, c, initial) {
      if (detached || !isActive()) return;
      if (editing) commit(true);
      var cell = cellAt(r, c);
      if (!cell) return;
      var rect = cell.getBoundingClientRect();
      var cs = getStyle(cell);
      ...
      function onInputBlur() { if (editing && editing.input === input) commit(true, 'blur'); }
      function onInputChange() {
        if (opts.valueInput) opts.valueInput.value = input.value;
      }
      editing = { input: input, r: r, c: c, onBlur: onInputBlur, onInput: onInputChange };
    ...
    function teardown() {
      ...
      inp.removeEventListener('keydown', onEditorKey);
      inp.removeEventListener('blur', record.onBlur);
      inp.removeEventListener('input', record.onInput);
    ...
    function stopFillDrag(commit) {
      document.removeEventListener('mousemove', onFillMove);
      document.removeEventListener('mouseup', onFillUp);
      clearFillPreview();
      if (commit && fillDrag && fillDrag.dir != null) commitFill();
      fillDrag = null;
    }
    function onFillUp() { stopFillDrag(true); }
    ...
    fillHandle.addEventListener('mousedown', onFillDown);
    ...
    var api = {
      ...
      detach: function (commitOpen) {
        if (detached) return;
        detached = true;
        if (editing) {
          if (commitOpen === false) teardown();
          else commit(true);
        }
        stopFillDrag(false);
        fillHandle.removeEventListener('mousedown', onFillDown);
        removeFillHandle();
        ...
        if (editorPointer) { editorPointer.end(); editorPointer = null; }
        internalCopy = null;
        var index = attachments.indexOf(api);
        if (index >= 0) attachments.splice(index, 1);
        if (wrapper._cellsEditApi === api) delete wrapper._cellsEditApi;
      },
  ...
  function destroy() {
    if (destroyed) return;
    destroyed = true;
    attachments.slice().forEach(function (api) { api.detach(false); });
    attachments.length = 0;
  }

  return { attach: attach, setRaw: setRaw, parseClipboard: parseClipboard, destroy: destroy };
  }

  function installProduction(root) {
    var S = root.SDocs;
    if (S.__cellsEditInstance) return S.__cellsEditInstance;
    var instance = create({
      window: root,
    ...
    S.__cellsEditInstance = instance;
    S.cellsEdit = instance;
    return instance;
  }
