...
(function (exports) {
  ...
  function createCodeFocus(S, config) {
  if (!S) return null;
  config = config || {};

  function readerRoot() {
    if (typeof config.root === 'function') return config.root();
    return config.root || S.renderedEl || document.getElementById('_sd_rendered');
  }

  function setHTML(node, html) {
    if (config.setHTML) config.setHTML(node, String(html));
    else node.innerHTML = html;
  }

  function controlEnabled(name) {
    return !config.controls || config.controls[name] !== false;
  }
  ...
  function loadStructural(lang) {
    ...
    langCache[key] = new Promise(function (resolve) {
      ...
      var base = config.structuralAssetBase || '/public/sdocs-code-lang/';
      s.src = base + key + '.js' + (SELF_V && !config.structuralAssetBase ? '?' + SELF_V : '');
  ...
  function updateThemeBtn() {
    ...
    setHTML(btn, dark ? SUN_ICON : MOON_ICON);
  ...
  function allComments() {
    if (config.comments === false) return [];
    return window.SDocComments ? window.SDocComments.getComments(S.currentMeta) : [];
  }
  ...
  function onCopyClick(e) {
    ...
    navigator.clipboard.writeText(sectionText(h) + '\n').then(function () {
      ...
      setHTML(btn, CHECK_ICON);
      setTimeout(function () { if (btn) setHTML(btn, prev); }, 1200);
    });
  }
  ...
  function buildModalChrome() {
    ...
    setHTML(topbar,
      ...
      +   (controlEnabled('copy') ? '<button type="button" class="sdoc-code-focus-btn" data-act="copy" title="Copy code" aria-label="Copy code">' + COPY_ICON + '</button>' : '')
      +   (config.comments === false ? '' : '<button type="button" class="sdoc-code-focus-btn" data-act="comment" title="Comment mode" aria-label="Comment mode" aria-pressed="false">' + COMMENT_ICON + '</button>')
      +   (controlEnabled('download') ? '<button type="button" class="sdoc-code-focus-btn" data-act="download" title="Download file" aria-label="Download file">' + DOWNLOAD_ICON + '</button>' : '')
  ...
  function mountFile(sourcePre, opts) {
    ...
    setHTML(methodTab, COMMENT_ICON);
    ...
    if (opts && opts.comment && config.comments !== false) setCommenting(true);

    highlightThenRender(srcCode.className || '');
  }

  function open(sourcePre, opts) {
    if (exports.active && exports.active !== api) exports.active.close();
    if (modal) close();
  ...
  function syncJumpNote() {
    ...
    setHTML(note, lucide('<line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/>', 12));
  ...
  function buildFileInfo() {
    ...
    var note = hasLocalRow
      ...
      : '';
    setHTML(card, '<div class="sdoc-cf-firows">' + rows + '</div>' + note);
    return card;
  }
  ...
  function addSectionCommentCopies() {
    ...
    for (var h = 0; h < folds.length; h++) {
      ...
      setHTML(btn, COPY_ICON + '<span>with comments</span>');
  ...
  function saveComposer(composerRow) {
    ...
    if (spec.editId) {
      updateAnyComment(spec.editId, text);
    } else {
      var prefs = readCommentPrefs();
  ...
  // Delete any comment, routing to the store that owns its kind.
  function removeAnyComment(id) {
    var c = commentById(id);
    if (c && c.kind === 'inline' && window.SDocComments) {
      S.currentMeta = window.SDocComments.removeComment(S.currentMeta, id);
      if (S.syncAll) S.syncAll('comment');
      loadComments();
    } else {
  ...
  // Update any comment's text, routing to the store that owns its kind.
  function updateAnyComment(id, text) {
    var c = commentById(id);
    if (c && c.kind === 'inline' && window.SDocComments) {
      S.currentMeta = window.SDocComments.updateComment(S.currentMeta, id, { text: text });
      if (S.syncAll) S.syncAll('comment');
      loadComments();
    } else {
  ...
  function ensureSelPopover() {
    ...
    setHTML(selPopover, COMMENT_ICON);
  ...
  function onCommentClick(e) {
    ...
    if (!btn) {
      // Click a card body or a pill to edit it, the same affordance as the
      // markdown card.
  ...
  // Open the composer pre-filled over an existing note, replacing its card/pill.
  // Works for any kind: a code note resolves by its source line; a prose inline
  // note resolves by the line its quote sits on.
  function startEdit(id) {
    var c = commentById(id);
    if (!c) return;
    var ln = (c.kind === 'inline') ? lineOfQuote(c.quote) : (CC ? CC.resolveLine(c, srcLines) : c.line);
    if (ln < 0) return;
    openComposer({ kind: c.kind, line: ln, endLine: c.endLine, quote: c.quote, anchorText: c.anchorText, editId: id });
  }
  ...
  function setCommenting(on) {
    if (config.comments === false) on = false;
    commenting = on;
  ...
  api = { open: open, close: close, destroy: close, openWalkthrough: openWalkthrough };
  return api;
  }

  exports.create = createCodeFocus;
})(typeof module !== 'undefined' && module.exports
  ? module.exports
  : (window.SDocCodeFocus = window.SDocCodeFocus || {}));
