...
Team checkout starts with the current active member count. Invitation acceptance is allowed while the configured Team `maxMembers` limit has capacity. Acceptance and member removal then attempt an immediate Stripe quantity update. Each committed membership change receives its own idempotency key, so retrying that change is safe while a later change can return to an earlier seat count. Every change also queues a durable reconciliation job. The job has a separate Stripe idempotency key and reads the current member count when it runs, so it repairs a failed update or two overlapping membership changes without replaying an earlier target quantity. Reconciliation reads the subscription again and updates its first item with prorations.
