...
Point `sdoc` at a source file and it opens as a highlighted listing: `sdoc app.rb`, `sdoc server.js`, `sdoc main.go`. The language is chosen from the extension, the file travels in the URL like any document, and nothing is uploaded. Opening a file gives a fullscreen view with a line-number gutter, soft wrap, language-aware folding (collapse a whole method or class), and per-section copy. Comment mode lets you leave notes anchored to a line or a whole method, to walk someone through the code or flag what to review. The notes live in the browser rather than the file, and a copy-with-comments action lifts the source out with the notes attached.
