...
This is not a unique identifier. It groups you with every other person who first visited that same week. Alongside the cohort, each visit also records a coarse device label (desktop / mobile / tablet), browser family (Chrome, Safari, etc.), a referrer category parsed from the standard HTTP headers, and the local hour and weekday from your device's clock at the time of the visit. If the entry URL contains a source label such as `?src=email-launch`, that lowercase label is also recorded. Recognized X, YouTube, and LinkedIn referrers supply a source label when the URL does not. None of these fields identifies you individually. Every 15 minutes, buffered visits are written to a local SQLite database. The dashboard at [smalldocs.org/analytics](https://smalldocs.org/analytics) shows visit counts per cohort per week. Its [Sources page](https://smalldocs.org/analytics/sources) creates a weekly chart and table for each source label after its first recorded visit. Source-attributed visits remain part of the ordinary visit and short-link totals.
