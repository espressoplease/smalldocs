...
(function (root, factory) {
  ...
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
    ...
    root.SDocCellsUI = api;
    // The application keeps its existing window.SDocs adapter. SDK customers
    // call create() with an instance-owned state object instead.
    if (root.SDocs && root.SDocCells && root.SDocCellsFormula && root.SDocCellsController) {
  ...
  var CELLS_CONTROLLER = options.controller;
  if (!window || !document) throw new Error('Cells UI requires window and document');
  if (!CELLS) throw new Error('Cells UI requires the cells model');
  if (!FORMULA) throw new Error('Cells UI requires the formula engine');
  if (!CELLS_CONTROLLER) throw new Error('Cells UI requires the workbook controller');
  var navigator = options.navigator || window.navigator;
  var ResizeObserver = options.ResizeObserver || window.ResizeObserver;
  var requestAnimationFrame = options.requestAnimationFrame || window.requestAnimationFrame;
  var cancelAnimationFrame = options.cancelAnimationFrame || window.cancelAnimationFrame;
  var setTimeout = options.setTimeout || window.setTimeout.bind(window);
  var Blob = options.Blob || window.Blob;
  var URL = options.URL || window.URL;
  var capabilities = options.capabilities || {};
  var controls = options.controls || {};
  var resizeClassTarget = options.resizeClassTarget || document.body;
  var sizeObservers = [];
  var pendingFrames = [];
  ...
  function setKnownHTML(element, html) {
    if (typeof options.setKnownHTML === 'function') options.setKnownHTML(element, html);
    else element.innerHTML = html;
  }
  function replaceSvg(svg, html) {
    var holder = document.createElement('span');
    setKnownHTML(holder, html);
  ...
  var SELF_V = options.assetVersion != null
  ...
  function loadCellsFeature(name) {
    if (typeof options.loadFeature === 'function') return options.loadFeature(name);
  ...
  if (options.installMarkedExtension !== false && typeof window.marked !== 'undefined' && window.marked && typeof window.marked.use === 'function') {
    window.marked.use({
  ...
  function flashTick(btn) {
    ...
    replaceSvg(svg, tickIcon(size));
    btn._tickTimer = setTimeout(function () {
      var cur = btn.querySelector('svg');
      if (cur) replaceSvg(cur, copyIcon(size));
      btn._tickTimer = null;
    }, 1500);
  }
  ...
  function copyButton(extraClass, label) {
    ...
    setKnownHTML(b, copyIcon(13) + '<span class="sdoc-cells-copy-label">' + label + '</span>');
    return b;
  }
    ...
    function paint() {
      ...
      for (var c = 0; c < renderCols; c++) {
        ...
        if (c < cols) {                                  // sort only real columns
          ...
          if (sorted) setKnownHTML(cur, sorted === 'asc' ? SORT_UP_SVG : SORT_DOWN_SVG);
          var nxt = document.createElement('span');
          nxt.className = 'sdoc-cells-sort-next';
          setKnownHTML(nxt, !sorted ? SORT_UP_SVG
  ...
  function processCells(container) {
    ...
    var inputs = Array.prototype.map.call(nodes, function (codeEl) {
      ...
      if (typeof options.sourceForCode === 'function') {
        source = options.sourceForCode(codeEl, pre, source);
      }
      return {
        source: source,
  ...
  return {
    ...
    destroy: function () {
      ...
      if (typeof cancelAnimationFrame === 'function') {
        pendingFrames.splice(0).forEach(function (id) { cancelAnimationFrame.call(window, id); });
      } else pendingFrames.length = 0;
      activeResizeCleanups.slice().forEach(function (cleanup) { cleanup(); });
    },
  };
  }

  function installProduction(root) {
    if (root.SDocs.__cellsUiInstance) return root.SDocs.__cellsUiInstance;
    var instance = create({
      window: root,
      ...
      controller: root.SDocCellsController,
      installMarkedExtension: true,
    });
    root.SDocs.__cellsUiInstance = instance;
    return instance;
  }
