...
module.exports = function(harness) {
  ...
  console.log('\n── Inline-Document Handoff Tests ──────────────\n');

  // Launcher limits vary. SmallDocs does not guess a universal budget: every
  // inline document takes a local bootstrap path, while URLs that carry no
  // document keep opening directly. See openBrowser in cli/lib/io.js.

  test('shouldBootstrapUrl: inline documents use the bootstrap at any size', () => {
    assert.strictEqual(cli.shouldBootstrapUrl('https://smalldocs.org/#md=x'), true);
    assert.strictEqual(cli.shouldBootstrapUrl('https://smalldocs.org/#local=e30&md=x'), true);
    assert.strictEqual(cli.shouldBootstrapUrl('https://smalldocs.org/#md=' + 'a'.repeat(40000)), true);
  });

  test('shouldBootstrapUrl: pages and short links still open directly', () => {
    assert.strictEqual(cli.shouldBootstrapUrl('https://smalldocs.org/'), false);
    assert.strictEqual(cli.shouldBootstrapUrl('https://smalldocs.org/library'), false);
    assert.strictEqual(cli.shouldBootstrapUrl('https://smalldocs.org/s/abc123'), false);
    assert.strictEqual(cli.shouldBootstrapUrl('not a URL'), false);
  });
  ...
  test('buildHopHtml: redirects to the full URL', () => {
    ...
    assert.ok(html.indexOf('continue manually') !== -1, 'a no-JavaScript fallback must be present');
  });

  test('buildHopHtml: the URL cannot escape the script or fallback link', () => {
    const html = cli.buildHopHtml('https://smalldocs.org/#md=x</script><img src=x>&q="quoted"');
    assert.strictEqual(html.indexOf('</script><img'), -1, 'script literal must stay open');
    assert.ok(html.indexOf('\\u003c/script>') !== -1);
    assert.strictEqual(html.indexOf('href="https://smalldocs.org/#md=x</script>'), -1);
    assert.ok(html.indexOf('&lt;/script&gt;&lt;img src=x&gt;&amp;q=&quot;quoted&quot;') !== -1);
  });
};
