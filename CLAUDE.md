...
- `open/` - private bootstrap pages for inline documents opened by the CLI. A document rides in the URL hash, and external browser launch paths can truncate it before the browser sees it (for example, Chromium's 32 KB process-singleton message on Linux, or the 8,191-character `cmd.exe` command string used on Windows). `openBrowser()` in `cli/lib/io.js` does not turn either transport limit into a SmallDocs size cap: every inline `#md=` document is written into an `open-<random>.html` bootstrap here (mode 0600), and the browser follows the full URL internally. Ordinary pages and short links open directly. Each run prunes bootstrap files older than five minutes. If the write fails, the direct URL is used, matching older CLI behaviour.
