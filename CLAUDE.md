...
- **Frontend**: split across `public/`:
  ...
  - `sdocs-toc.js` - Contents panel: toggleable TOC overlay built from the rendered h1-h4 headings (desktop, read/comment modes, >=3 headings). Fold state mirrors the document's `.md-section-body.open` classes two-way (panel chevrons click the real heading; a MutationObserver reflects document toggles back). Entry clicks go through `S.revealSection`; open state persists in localStorage (`sdocs-toc-open`)
...
- **Playwright tests**: `npx playwright test test/write-mode.spec.js` - write mode editor tests
  ...
  - `test/toc.spec.js` - Contents panel: >=3-heading gate, nested list build, localStorage persistence, two-way fold mirroring, entry click reveal + scroll, scrollspy, mode gating
