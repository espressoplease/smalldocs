...
- **Frontend**: split across `public/`:
  ...
  - `sdocs-design-partner.js` - "become a design partner" nudge. A top strip (`#_sd_dp-banner`) that invites visitors to leave a first name + work email for the in-progress teams version. There is NO dismiss × on either surface: the doc net is count-gated (shows once at the 5th distinct doc, once at the 25th, then never) and the homepage is a standing strip, so a × does no real work - only submitting hides it (sets `_sd_dp_submitted`, everywhere). `init('home')` shows it ALWAYS on the homepage (a standing "building for teams" strip - still counts loads for the conversion record, but the count never gates it); `init('doc')` shows it on DOCUMENT pages only once someone has clearly used SmallDocs - gated on DISTINCT documents opened (by `#md` fingerprint, so it catches CLI users who never see the homepage), firing on the 5th and again on the 25th. Fields are first name + email, deliberately with NO email validation client-side (free/personal domains like gmail are fine; only a blank-field guard). The submit button is labelled "Learn More". The SAME form is also mounted at the top of the info side panel via `SDocsDesignPartner.mount(el, source)` (a standing copy, source-tagged "info panel"; shows a thank-you if already submitted). Submissions POST to `/api/teams-interest` (same pipe as the homepage Teams form); the first name rides in the message field (no schema change to the tracked teams DB) alongside a source tag. The server stores the row in `teams_interest.db` AND fires an email ping via `teams/notify.js` - but that ping only sends if `NOTIFY_SMTP_USER`/`NOTIFY_SMTP_PASS`/`NOTIFY_EMAIL_TO` are set on the server; with no SMTP config it's a silent no-op (the row is still stored). On mobile (<=900px) it stacks into two layers - info line, then the input row - with 16px inputs so iOS doesn't auto-zoom on focus. Counting is client-side (localStorage); submitting sets `_sd_dp_submitted` and suppresses the strip everywhere. Loaded by both `index.html` (doc app) and `homepage.html`; the homepage carries its own copy of the CSS inline (it loads no shared stylesheet) - the app copy lives in `css/layout.css`, keep the two in sync
