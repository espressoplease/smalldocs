...
Ahead of that branch sits `checkoutRoot()` in the same module: a `.git` at or above the realpath-resolved package root means the running CLI is a git working tree, and neither upgrade command may touch it (`npm i -g` would shadow it with a second copy; `install.sh` would replace `~/.sdocs/cli` outright, including when that path is a symlink onto a clone). A checkout makes `maybeUpdateBinary()` return silently, `sdoc upgrade` print `git pull` instead of running anything, `sdoc setup` skip the auto-install question, and `sdoc auto-update on` refuse. Working on the CLI from a clone is therefore safe with `autoInstallUpdates` left on.
